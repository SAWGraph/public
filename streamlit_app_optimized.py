...
with st.sidebar:
    ...
    query_limit = st.slider(
        ...
        value=50,
        step=10,
        help="Limit the number of results to improve query performance. Start with a lower value if queries are slow."
    )
    
    # Add timeout warning
    st.info("💡 Tip: If queries are taking too long, try reducing the result limit or selecting fewer industries.")

# Query builders
def build_q1_query(limit=50):
...
def build_facilities_query(limit=200):
    ...
    with st.spinner("Executing queries..."):
        try:
            ...
            if query_type.startswith("Q1"):
                ...
                progress_text.text("Executing Q1: Fetching sample points...")
                progress_bar.progress(10)
                
                q1 = build_q1_query(limit=query_limit)
                samplepoints = execute_query(q1)
                
                progress_text.text("Fetching facilities...")
                progress_bar.progress(50)
                
                # Execute facilities query
                q2 = build_facilities_query(limit=200)
                ...
                progress_text.text("Executing Q2: Fetching sample points in counties...")
                progress_bar.progress(10)
                
                q1 = build_q2_query(limit=query_limit)
                samplepoints = execute_query(q1)
                ...
                progress_text.text("Fetching county boundaries...")
                ...
                progress_text.text("Executing Q3: Fetching surface water bodies...")
                progress_bar.progress(10)
                
                q1 = build_q3_query(limit=query_limit)
                ...
                progress_text.text("Executing Q4: Fetching downstream water bodies...")
                progress_bar.progress(10)
                
                q1 = build_q4_query(limit=query_limit)
                surfacewater = execute_query(q1)
            ...
            st.success(f"Query executed successfully! Retrieved {len(st.session_state.query_results.get('samplepoints', st.session_state.query_results.get('surfacewater', [])))} results.")
            
        except Exception as e:
            progress_text.empty()
            progress_bar.empty()
            st.error(f"Error executing query: {str(e)}")
            st.info("Try reducing the result limit or selecting fewer industries.")
